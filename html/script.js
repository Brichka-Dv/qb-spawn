...
function setupLocations(locations, myHouses) {
    ...
    setTimeout(function(){
        $(parent).append('<div class="location" id="location" data-location="current" data-type="current" data-label="Last Location"><span id="slot-icon" class="material-icons">place</span><span id="slot-name"><span id="current-location">Last Location</span></span></div>');
        
        $.each(locations, function(index, location){
            $(parent).append('<div class="location" id="location" data-location="'+location.location+'" data-type="normal" data-label="'+location.label+'"><span id="slot-icon" class="material-icons">add_location</span><span id="slot-name"><span id="'+location.location+'">'+location.label+'</span></span></div>')
        });

        if (myHouses != undefined) {
            $.each(myHouses, function(index, house){
                $(parent).append('<div class="location" id="location" data-location="'+house.house+'" data-type="house" data-label="'+house.label+'"><span id="slot-icon" class="material-icons">house</span><span id="slot-name"><span id="'+house.house+'">'+house.label+'</span></span></span></div>')
            });
        }
...
function setupApps(apps) {
    ...
    $.each(apps, function(index, app){
        $(parent).append('<div class="location" id="location" data-location="'+app.name+'" data-type="appartment" data-label="'+app.label+'"><span id="slot-icon" class="material-icons">apartment</span><span id="slot-name"><span id="'+app.name+'">'+app.label+'</span></span></div>')
    });
